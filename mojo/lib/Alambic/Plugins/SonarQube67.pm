...
package Alambic::Plugins::SonarQube67;
...
sub run_plugin($$) {
  ...
  # say "COUCOU say";
    ...
    push(
      @{$ret{'log'}},
      "[Plugins::SonarQube67] Using provided proxy [$proxy_url]."
    );
  }
  else {
    ...
    push(
      @{$ret{'log'}},
      "[Plugins::SonarQube67] No proxy defined [$proxy_url]."
    );
  }
  ...
  if ($content->{'valid'}) {
    push(
      @{$ret{'log'}},
      "[Plugins::SonarQube67] Authentication successful on server."
    ...
    push(
      @{$ret{'log'}},
      "[Plugins::SonarQube67] Authentification failed on server."
    );
  }
  ...
  push(
    @{$ret{'log'}},
    "[Plugins::SonarQube67] Get issues from [${url_issues}]."
  );
  ...
  $repofs->write_plugin('SonarQube67', $project_id . "_sq_issues_blocker.csv",
    $csv_out);
  ...
  $repofs->write_plugin('SonarQube67', $project_id . "_sq_issues_critical.csv",
    $csv_out);
  ...
  $repofs->write_plugin('SonarQube67', $project_id . "_sq_ref_rules.json",
    encode_json(\%rules));
  ...
  push(
    @{$ret{'log'}},
    "[Plugins::SonarQube67] Got [" . scalar @{$content->{'rules'}} . "] rules."
  );
  ...
  $repofs->write_plugin('SonarQube67', $project_id . "_sq_issues_major.csv",
    $csv_out);
  ...
  push(
    @{$ret{'log'}},
    "[Plugins::SonarQube67] Get resources from [${url_info}]."
  );
  ...
  $repofs->write_plugin('SonarQube67', $project_id . "_sq_info.csv", $csv_out);
  ...
  $repofs->write_plugin('SonarQube67', $project_id . "_sq_metrics.csv",
    $csv_out);

  # Now execute the main R script.
  push(@{$ret{'log'}}, "[Plugins::SonarQube67] Executing R main file.");
  my $r = Alambic::Tools::R->new();
  push(
    @{$ret{'log'}},
    @{$r->knit_rmarkdown_inc('SonarQube67', $project_id, 'sonarqube45.Rmd')}
  );
  ...
  foreach my $fig (sort @figs) {
    push(@{$ret{'log'}}, "[Plugins::SonarQube67] Executing R fig file [$fig].");
    @{$ret{'log'}} = (
      @{$ret{'log'}},
      @{$r->knit_rmarkdown_html('SonarQube67', $project_id, $fig)}
    );
  }
...
B<Alambic::Plugins::SonarQube67> - A plugin to fetch information from a 
SonarQube 6.7.x instance.

=head1 DESCRIPTION

B<Alambic::Plugins::SonarQube67> retrieves information from a SonarQube 6.7.x instance.
