...
package Alambic::Plugins::SonarQubeIo;
...
my %conf = (
  "id"   => "SonarQubeIo",
  "name" => "SonarQube cloud platform",
  "desc" => [
    "Retrieves information from the SonarQube Cloud platform (i.e. metrics and violations), and visualises them.",
    "Check the documentation for this plugin on the project wiki: <a href=\"http://alambic.io/Plugins/Pre/SonarQubeIo.html\">http://alambic.io/Plugins/Pre/SonarQubeIo.html</a>."
  ],
  "type"    => "pre",
...
#      'sonarqube_coverage.html' => "Bar plot of the different coverage metrics of SonarQube",
  ...
  "provides_recs" => [],
  "provides_viz"  => {"SonarQubeIo.html" => "SonarQubeIo",},
);
...
sub run_plugin($$) {
  ...
  my $sonar_url     = "https://sonarcloud.io";
  ...
  if ($proxy_url =~ m!^default!i) {
    ...
    push(
      @{$ret{'log'}},
      "[Plugins::SonarQubeIo] Using default proxy [$proxy_http] and [$proxy_https]."
    );
  }
  elsif ($proxy_url =~ m!\S+!) {
    ...
    push(
      @{$ret{'log'}},
      "[Plugins::SonarQubeIo] Using provided proxy [$proxy_url]."
    );
  }
  else {
    ...
    push(
      @{$ret{'log'}},
      "[Plugins::SonarQubeIo] No proxy defined [$proxy_url]."
    );
  }
  ...
  if ($content->{'valid'}) {
    push(
      @{$ret{'log'}},
      "[Plugins::SonarQubeIo] Authentication successful on server."
    ); 
  }
  else {
    push(
      @{$ret{'log'}},
      "[Plugins::SonarQubeIo] Authentification failed on server."
    ); 
  }
  ...
  push(
    @{$ret{'log'}},
    "[Plugins::SonarQubeIo] Get issues from [${url_issues}]."
  ); 
  ...
  push(
    @{$ret{'log'}},
    "[Plugins::SonarQubeIo] Got [" . scalar @{$content->{'rules'}} . "] rules."
  );
  ...
  my $url_res
    ...
    . $sonar_project
    . '&format=json&metricKeys='
    . $metrics_csv;
  push(
    @{$ret{'log'}},
    "[Plugins::SonarQubeIo] Get resources from [${url_res}]."
  ); 
  $content = $ua->get($url_res)->res->json;

  # Store all metrics with their Alambic names instead of SQ names.
  foreach my $m (@{$content->{'component'}{'measures'}}) {
    if (exists($conf{'provides_metrics'}{$m->{'metric'}})) {
      if (exists($m->{'periods'})) {
        $ret{'metrics'}{$conf{'provides_metrics'}{$m->{'metric'}}} = $m->{'periods'}[0]{'value'};
      } elsif (exists($m->{'value'})) {
        $ret{'metrics'}{$conf{'provides_metrics'}{$m->{'metric'}}} = $m->{'value'};
      } else {
        $ret{'metrics'}{$conf{'provides_metrics'}{$m->{'metric'}}} = 'Error';
      }
    }
  }
  push(
    @{$ret{'log'}},
    "[Plugins::SonarQubeIo] Got [" . scalar
  ...
  push(
    @{$ret{'log'}},
    "[Plugins::SonarQubeIo] Get resources from [${url_info}]."
  );
  ...
  push(
    @{$ret{'log'}},
    @{$r->knit_rmarkdown_inc('SonarQubeIo', $project_id, 'SonarQubeIo.Rmd')}
  );
  ...
  foreach my $fig (sort @figs) {
    push(@{$ret{'log'}}, "[Plugins::SonarQubeIo] Executing R fig file [$fig].");
    @{$ret{'log'}} = (
      @{$ret{'log'}},
      @{$r->knit_rmarkdown_html('SonarQubeIo', $project_id, $fig)}
    );
  }
  @{$ret{'log'}} = (
    @{$ret{'log'}},
    @{$r->knit_rmarkdown_images('SonarQubeIo', $project_id, 
        'sonarqube_violations_bar.r', ['sonarqube_violations_bar.svg'])}
  );
...
B<Alambic::Plugins::SonarQubeIo> - A plugin to fetch information from a 
SonarQube 4.5.x instance.

=head1 DESCRIPTION

B<Alambic::Plugins::SonarQubeIo> retrieves information from a SonarQube 4.5.x instance.
...
For the complete configuration see the user documentation on the web site: L<https://alambic.io/Plugins/Pre/SonarQubeIo.html>.

=head1 SEE ALSO

L<https://alambic.io/Plugins/Pre/SonarQubeIo.html>, L<https://www.sonarqube.org>,
