...
sub run_plugin($$) {
    ...
    for( my $i = 0 ; $i<scalar(@{$commits_weekly->{'all'}}) ; $i++ ) {
...
B<Alambic::Plugins::GitHubProject> - Retrieves data and builds a summary of data for a Github project, along with
SCM information (commits, etc.).
...
=item * github_url The URL of the remote Github instance, e.g. https://api.github.com.

=item * github_user The user the analysed project belongs to in the Github forge, 
e.g. 'bbaldassari' if the full project name is bbaldassari/Alambic.

=item * github_repo The repository of the project to analyse in the Github forge, 
e.g. 'Alambic' if the full project name is bbaldassari/Alambic.
