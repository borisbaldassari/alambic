...
my %conf = (
  ...
  "params"  => {
    "project_grim" =>
      "The ID used to identify the project on the dashboard server. Note that it may be different from the id used in the PMI.",
  },
  "provides_cdata" => [],
  "provides_info"  => [],
  "provides_data"  => {
    "import_mls.json" =>
      "The original file of current metrics from the Eclipse dashboard server (JSON).",
  ...
  "provides_figs" => {
    ...
    "mls_evol_people.html" =>
      "Evolution of people involved in mailing lists (HTML)",
    "mls_evol_sent.html" => "Evolution of emails sent on mailing list (HTML)",
  },
  "provides_recs" => ["MLS_SENT",],
  "provides_viz"  => {"eclipse_mls.html" => "Eclipse MLS",},
);
...
sub _retrieve_data($$$) {
  ...
  my $url
    = "http://dashboard.eclipse.org/data/json/5y/"
    . $project_grim
    . "-mls-prj-static.json";

  push(@log,
    "[Plugins::EclipseMls] Starting retrieval of data for [$project_id] url [$url]."
  );
...
sub _compute_data($$$) {
  ...
  foreach my $metric (keys %{$metrics_old}) {
    if (exists($conf{'provides_metrics'}{uc($metric)})) {
      $metrics_new->{$conf{'provides_metrics'}{uc($metric)}}
        = $metrics_old->{$metric};
    }
  }

  # Write mls metrics json file to disk.
  $repofs->write_output($project_id, "metrics_mls.json",
    encode_json($metrics_new));
  ...
  $csv_out
    = "date,sent_response,id,senders_init,senders_response,sent,threads,repositories,senders,unixtime\n";
  ...
  @log = (
    @log,
    @{$r->knit_rmarkdown_inc('EclipseMls', $project_id, 'eclipse_mls.Rmd')}
  );

  # And execute the figures R scripts.
  my @figs
    = ("mls_evol_summary.rmd", "mls_evol_people.rmd", "mls_evol_sent.rmd",);
  ...
  return {"metrics" => $metrics_new, "recs" => \@recs, "log" => \@log,};
}
