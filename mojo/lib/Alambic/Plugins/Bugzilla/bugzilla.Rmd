...
```{r evol, echo=FALSE, message=FALSE, results='asis'}
p <-dygraph(evol.monthly[,1],
        main = paste('Issues submissions for ', project.id, sep=''),
        width = 800, height = 250 ) %>%
      dyRangeSelector()
p
# g <- gvisColumnChart(as.data.frame(evol.weekly, options=list(legend="{position: 'none'}", width="automatic", height="300px")))
# plot(g)
```
...
```{r components_plot, echo=FALSE, results='asis'}
g <- gvisColumnChart(components.sorted, options=list(legend="{position: 'none'}", width="automatic", height="300px"))
plot(g)
```
</div></div>
...
```{r versions_plot, echo=FALSE, results='asis'}
g <- gvisColumnChart(versions.sorted, options=list(legend="{position: 'none'}", width="automatic", height="300px"))
plot(g)
```
</div></div>
