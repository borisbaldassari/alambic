...
f <- function_m(x, output) {
  id <- paste("\n\n## ", x[1], sep=" ")
  name <- paste("Name: ", x[2], sep=" ")
  name <- paste("Value: ", metrics[metrics[[,1]==x[1]], 2], sep=" ")
  desc <- paste("Description: ", x[3], sep=" ")
  paste(id, name, desc, sep="\n\n") 
}
cat( paste( apply(metrics_ref, 1, f), sep="" ) )
```
...
f <- function_a(x, output) {
  id <- paste("\n\n## ", x[1], sep=" ")
  name <- paste("Name: ", x[2], sep=" ")
  name <- paste("Value: ", attrs[attrs[[,1]==x[1]], 2], sep=" ")
  desc <- paste("Description: ", x[3], sep=" ")
  paste(id, name, desc, sep="\n\n") 
}
cat( paste( apply(attrs_ref, 1, f), sep="" ) )
```
