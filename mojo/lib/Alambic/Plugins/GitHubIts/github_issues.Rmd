...
The project was created on ``r info[['ITS_CREATED_AT']]`` and was last updated on ``r info[['ITS_UPDATED_AT']]``.
...
milestones <- milestones[order(milestones$Milestone, decreasing = F),]
...
t <- lapply(milestones$Milestone,
		   function(x) paste(' <tr><td>', milestones[x,1], '</td><td>', trimws(milestones[x,4], which='both'), '</td><td>',
		           trimws(milestones[x,2], which='both'), '</td><td>', trimws(milestones [x,3], which='both'), '</td></tr>', sep=''))
t <- paste(t, collapse="")
cat(t)
```
</table>
