...
* It is composed of a total of ``r forum$topic_count`` threads and ``r forum$posts_count`` posts.
* Last post was made on ``r index(last(posts.xts))`` by user ``r forum$last_post_id`` and was about\\ ``r last(posts.xts)$subject``.
* Category of the forum is ``r forum$category_id`` ([`r forum$category_url`](see category)
...
threads.table$subject <- paste('<a href="', threads$html_url, '">', threads$subject, '</a>', sep='')
...
posts.table$subject <- paste('<a href="', posts$html_url, '">', posts$subject, '</a>', sep='')
...
<img src="/projects/`r project.id`/EclipseForums/eclipse_forums_hist.svg" frameborder="0" style="width: 100%; height: 460px" />
