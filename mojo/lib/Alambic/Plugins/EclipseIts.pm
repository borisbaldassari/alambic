...
my %conf = (
  ...
  "provides_data"  => {
    "import_its.json" =>
      "The original file of current metrics downloaded from the Eclipse dashboard server (JSON).",
  ...
  "provides_recs" => ["ITS_OPEN_BUGS", "ITS_CLOSERS",],
...
sub _retrieve_data($$$) {
  ...
  push(@log,
    "[Plugins::EclipseIts] Starting retrieval of data for [$project_id] url [$url]."
  );
...
sub _compute_data($$$) {
  ...
  $repofs->write_output($project_id, "metrics_its.json",
  ...
  @log = (
    @log,
    @{$r->knit_rmarkdown_inc('EclipseIts', $project_id, 'eclipse_its.Rmd')}
  );

  # And execute the figures R scripts.
  my @figs = (
    'its_evol_summary.rmd', 'its_evol_changed.rmd',
    'its_evol_opened.rmd',  'its_evol_people.rmd',
  );
  ...
  if ($closed_old < (2 * $opened_old)) {
    push(
      @recs,
      {
        ...
        'src'      => 'EclipseIts',
        'desc' =>
          'During last 4 weeks, there has been twice as many opened bugs ('
          . $opened_old
          . ') as closed bugs ('
          . $closed_old
          . '). This may be ok '
    ...
    push(
      @recs,
      {
        ...
        'desc' => 'During past year, the number of people closing issues has '
          . ' fallen by '
          . $metrics_new->{'ITS_DIFF_NETCLOSERS_365'}
