...
file.out <- paste( project.id, "_sonarqube_violations_bar.svg", sep="")

svg(file.out, width=10, height=8)
ggplot(violations, aes(x=reorder(Severity, Values), y=Values, fill = factor(Severity))) +
    geom_bar(stat='identity') +
    geom_text(aes(label=Values), vjust=-0.2, size=4) +
