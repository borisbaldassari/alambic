...
my %conf = (
  ...
  "params"  => {
    "project_grim" =>
      "The project ID used to identify the project on the dashboard server. Note that it may be different from the id used in the PMI.",
  },
  "provides_cdata" => [],
  "provides_info"  => [],
  "provides_data"  => {
  ...
  "provides_recs" => ["SCM_CLOSE_BUGS",],
  "provides_viz"  => {"eclipse_scm.html" => "Eclipse SCM",},
...
sub _retrieve_data($) {
  ...
  push(@log,
    "[Plugins::EclipseScm] Starting retrieval of data for [$project_id] url [$url]."
  );
...
sub _compute_data($) {
  ...
  $repofs->write_output($project_id, "metrics_scm.json",
  ...
  @log = (
    @log,
    @{$r->knit_rmarkdown_inc('EclipseScm', $project_id, 'eclipse_scm.Rmd')}
  );

  # And execute the figures R scripts.
  my @figs = (
    'scm_evol_summary.rmd', 'scm_evol_lines.rmd',
    'scm_evol_people.rmd',  'scm_evol_commits.rmd',
  );
