...
ok( exists($ret->{'EclipsePmi'}), "Get names all contains EclipsePmi" )
    or diag explain $ret;
ok( $ret->{'EclipsePmi'} =~ m!^Eclipse PMI Wizard!, "Get names returns EclipsePmi name" )
    or diag explain $ret;

$ret = $wizards->get_conf_all();
ok( exists($ret->{'EclipsePmi'}), 
    "Get conf_all contains EclipsePmi" )
    or diag explain $ret;
ok( $ret->{'EclipsePmi'}{'name'} =~ m!^Eclipse PMI Wizard!, 
    "Get conf all returns EclipsePmi name." )
    or diag explain $ret;
ok( $ret->{'EclipsePmi'}{'id'} =~ m!^EclipsePmi!, 
...
$ret = $wiz->get_conf();
ok( $ret->{'name'} =~ m!^Eclipse PMI Wizard!, 
    "Get conf all returns EclipsePmi name." )
    or diag explain $ret;
ok( $ret->{'id'} =~ m!^EclipsePmi!, 
    "Get conf all returns EclipsePmi id." )
    or diag explain $ret;
