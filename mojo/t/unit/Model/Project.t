...
my $qm = [ {'mnemo' => 'ATTR1', 'active' => 'true', 'type' => 'attribute', 'children' => [{'mnemo' => 'PMI_ITS_INFO', 'active' => 'true', 'type' => 'metric'}, {'mnemo' => 'PMI_SCM_INFO', 'active' => 'true', 'type' => 'metric'}] } ];
...
my $plugins = {
	...
	"params"  => {
	    "project_pmi" => "The project ID used to identify the project on the PMI server. Look for it in the URL of the project on <a href=\"http://projects.eclipse.org\">http://projects.eclipse.org</a>.",
        },
	"provides_cdata" => [],
	"provides_info"  => [
...
ok( grep( /^\[Plugins::EclipsePmi\] Using Eclipse PMI infra at/, @{$ret} ) == 1, 
    "Log has Starting retrieval." ) or diag explain $ret;
ok( grep( /^\[Plugins::EclipsePmi\] Writing PMI json file to input./, @{$ret} ) == 1, 
    "Log has Writing json file to input." ) or diag explain $ret;
ok( grep( /^\[Plugins::EclipsePmi\] Writing PMI checks json file to output/, @{$ret} ) == 1, 
    "Log has Writing json file to output." ) or diag explain $ret;
ok( grep( /^\[Plugins::EclipsePmi\] Writing PMI checks csv file to output/, @{$ret} ) == 1, 
    "Log has Writing csv file to output." ) or diag explain $ret;
...
ok( exists($ret->{'PMI_ITS_INFO'}), "There should be a metric called PMI_ITS_INFO." ) or diag explain $ret;
ok( exists($ret->{'PMI_SCM_INFO'}), "There should be a metric called PMI_SCM_INFO." ) or diag explain $ret;
...
ok( exists($ret->{'metrics'}{'PMI_ITS_INFO'}), 
    "PMI_ITS_INFO metric exists in run after plugins." ) or diag explain $ret;
ok( exists($ret->{'metrics'}{'PMI_SCM_INFO'}), 
    "PMI_ITS_INFO metric exists in run after plugins." ) or diag explain $ret;
ok( $ret->{'recs'}[0]{'rid'} =~ m!PMI_EMPTY_TITLE! , 
    "recs[0] is PMI_EMPTY_TITLE." ) or diag explain $ret;
ok( $ret->{'recs'}[0]{'severity'} == 2 , 
    "recs[0] (PMI_EMPTY_TITLE) has severity 2." ) or diag explain $ret;
ok( $ret->{'recs'}[0]{'src'} =~ m!^EclipsePmi$!, 
    "recs[0] (PMI_EMPTY_TITLE) has src EclipsePmi." ) or diag explain $ret;
ok( $ret->{'recs'}[0]{'desc'} =~ m!^The title entry is empty in the PMI.$!, 
    "recs[0] (PMI_EMPTY_TITLE) has correct description." ) or diag explain $ret;
ok( $ret->{'info'}{'PMI_BUGZILLA_CREATE_URL'} =~ m!^https://bugs.eclipse.org/bugs/enter_bug.cgi\?product=CDT$!, 
    "PMI_BUGZILLA_CREATE_URL info has url." ) or diag explain $ret;
ok( $ret->{'info'}{'PMI_WIKI_URL'} =~ m!^http://wiki.eclipse.org/index.php/CDT$!, 
    "PMI_WIKI_URL info has url." ) or diag explain $ret;
ok( $ret->{'info'}{'PMI_MAIN_URL'} =~ m!^http://www.eclipse.org/cdt$!, 
    "PMI_MAIN_URL info has url." ) or diag explain $ret;
ok( $ret->{'info'}{'PMI_DOWNLOAD_URL'} =~ m!^http://www.eclipse.org/cdt/downloads.php$!, 
    "PMI_DOWNLOAD_URL info has url." ) or diag explain $ret;


note("Run qm.");
$ret = $project->run_qm($models); 
ok( grep( /Aggregating data/, @{$ret->{'log'}} ), 
    "After qm run log has aggregating data." ) or diag explain $ret;
ok( exists( $ret->{'attrs_conf'}{'ATTR1'} ),
...
ok( $ret->{'inds'}{'PMI_ITS_INFO'} == 5, 
    "PMI_ITS_INFO indicator value is 5." ) or diag explain $ret;
