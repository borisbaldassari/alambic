...
my $u = {
  'administrator' => {
    ...
    'passwd' =>
      '{X-PBKDF2}HMACSHA1:AAAD6A:5R2HIw==:3c7E0POr1PCmC7XQdahjgr/PDus=',
    'projects' => {},
    'id'       => 'administrator'
  }
};
...
ok($ret->{'id'} =~ m!^administrator!, "Get user admin returns correct id.")
  or diag explain $ret;
ok(
  $ret->{'email'} =~ m!^alambic.castalia.solutions!,
  "Get user admin returns correct email."
) or diag explain $ret;
ok(grep(/^Admin/, @{$ret->{'roles'}}),
  "Get user admin list of roles contains Admin")
  or diag explain $ret;
...
ok(exists($ret->{'administrator'}), "Get users returns administrator hash key.")
  or diag explain($ret);
ok(
  $ret->{'administrator'}{'name'} =~ m!^Administrator!,
