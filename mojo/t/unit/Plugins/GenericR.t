...
ok( grep( m!generic_r.pdf!, keys %{$conf->{'provides_data'}} ), "Conf has provides_data > generic_r.pdf" );
...
my $project = Alambic::Model::Project->new('modeling.sirius', 'Test project GenericR', 'TRUE', '',
  $plugins_conf);
...
ok( grep(/^\[Plugins::GenericR\] Executing R pdf markdown document./, 
	 @{$ret->{'log'}}) == 1, 
    "Checking if log contains R pdf exec.") or diag explain $ret;
ok( grep(/^\[Tools::R\] Exec \[Rsc.*generic_r.Rmd/, 
	 @{$ret->{'log'}}) == 1, 
    "Checking if log contains R code exec.") or diag explain $ret;
    
# Check that files have been created.
note( "Check that files have been created. ");
ok( -e "projects/modeling.sirius/output/modeling.sirius_generic_r.pdf", "Check that file generic_r.pdf exists." );
